  ...
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    ...
    if [ "${IF_COUCHDB}" == "couchdb" ]; then
      if [ "$CONSENSUS_TYPE" == "kafka" ]; then
        ...
        COMPOSE_FILES="-f $COMPOSE_FILE -f $COMPOSE_FILE_COUCH -f COMPOSE_FILE_CA"
      fi
    else
      if [ "$CONSENSUS_TYPE" == "kafka" ]; then
...
function networkDown() {
  # stop org3 containers also in addition to org1 and , in case we were running sample to add org3
  # stop kafka and zookeeper containers in case we're running with kafka consensus-type
  docker-compose -f $COMPOSE_FILE -f $COMPOSE_FILE_COUCH -f $COMPOSE_FILE_KAFKA -f $COMPOSE_FILE_ORG3 -f COMPOSE_FILE_CA down --volumes --remove-orphans
...
# (``peer0.org1.example.com`` & ``peer0..example.com``).  Secondly, we point to
