...
app.post("/givePermission",(req,res)=>{

	console.log("givePermission api")
	//var peers = req.body.peers;

		console.log(req.body)
	
   // var data=JSON.parse(req.body)
    var args=[req.body.candidateID, req.body.companyID]
	
	console.log("Arguments:",args)

var peers=["peer0.org1.example.com","peer1.org1.example.com"]	
/*	invoke.invokeChaincode(["peer0.org1.example.com"] , "mychannel", "mahindra-pilot", 'pushSamplingInfo', args, req.username, req.orgname).then(function(result,error){
		res.send(result);
	});
*/
	...
	if (!chaincodeName) {
        ...
        invoke.invokeChaincode(peers, channelName, chaincodeName, fcn, args, req.username, req.orgname).then(function(message){ 
			res.send(message);

	});                     
})

app.post("/pushOnbordingData",(req,res)=>{

	console.log("pushOnbordingData API")
...
var peers=["peer0.org1.example.com","peer1.org1.example.com"]	
	...
	logger.debug('==================== INVOKE ON CHAINCODE ==================');
       var peers =req.body.peers
        var chaincodeName = "mycc";
        var channelName = "mychannel";
        var fcn = "pushOnbordingData";
        logger.debug('channelName  : ' + channelName);
        logger.debug('chaincodeName : ' + chaincodeName);
        logger.debug('fcn  : ' + fcn);
        logger.debug('args  : ' + args);
	//let peers=["peer0.org1.example.com","peer1.org1.example.com"]
	console.log("_________________________________"+peers)
		...
		invoke.invokeChaincode(peers, channelName, chaincodeName, fcn, args, req.username, req.orgname).then(function(result){
			res.send(result);
		});
	
	})

	app.post("/updateCanditateData",(req,res)=>{

		console.log("updateCanditateData API")
...
console.log("_________________________________"+peers)
			...
			var args=[
				...
				data.Experiance.toString(),     
				data.RelievingDate.toString(),      
				data.WorkingSkillSet.toString(),       
			...
			invoke.invokeChaincode(peers, channelName, chaincodeName, fcn, args, req.username, req.orgname).then(function(result,error){
				res.send(result);
			});
		
		})

		app.get("/getCandidateBasicData/:candidateID",(req,res)=>{


			console.log(req.param)
		//    var data=JSON.parse(req.body.data)
			var args=[
				req.params.candidateID.toString(),
				]
		
			console.log("Arguments:",args)
			
			query.queryChaincode(req.body.peers, "mychannel", "hrChaincode", args, "getCandidateBasicData", req.username, req.orgname).then(function(result,error){
